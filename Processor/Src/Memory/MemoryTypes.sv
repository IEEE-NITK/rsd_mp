...
package MemoryTypes;
    ...
    import MicroArchConf::*;  // <-- NEW: for ThreadID / THREAD_NUM_BIT_WIDTH
...
`endif
    ...
    // For counting the latency of memory read/write access
    ...
    typedef struct packed {
        ...
        MemAccessSerial     nextMemReadSerial;   // Serial ID for next read request
        MemWriteSerial      nextMemWriteSerial;  // Serial ID for next write request
        logic               wr;
        ThreadID            tid;                 // NEW: SMT thread ID for this request
    } MemoryLatencySimRequestPath;

endpackage : MemoryTypes